...
/*
 * Check if the user is inside a November app directory (or a sub-directory)
 */
var novemberDir = function() {
  var files = Finder.in('.').lookUp().findFiles('package.json');
...
var fillTemplatePlaceholders = function(str, modelName) {
  ...
  str = str.replace(/{{x-camelcase}}/g, inflect.decapitalize(inflect.camelize(modelName)));
  str = str.replace(/{{x-dashed}}/g, inflect.dasherize(modelName));
  str = str.replace(/{{x-underscore}}/g, inflect.underscore(modelName));
  str = str.replace(/{{x-human}}/g, inflect.humanize(modelName));

  str = str.replace(/{{x-plural-camelcase}}/g, inflect.camelize(inflect.pluralize(modelName)));
